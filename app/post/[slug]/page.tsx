...
// Strip HTML tags for plain-text fallback descriptions
...
export async function generateMetadata({
  ...
  const description =
    post.meta_description || post.excerpt || stripHtml(post.content).slice(0, 160)
...
export default async function PostPage({
  ...
  const description =
    typedPost.meta_description ||
    typedPost.excerpt ||
    stripHtml(typedPost.content).slice(0, 160)
  ...
  const jsonLd = {
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': `${siteConfig.url}/post/${typedPost.slug}`,
    },
  }
  ...
  return (
    <article>
      ...
      <Link
        href="/"
        className="text-sm text-cactus-700 hover:underline mb-6 inline-block"
      >
