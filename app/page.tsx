...
export default async function HomePage() {
  ...
  return (
    <div className="pb-10">
      ...
      <section className="py-14 sm:py-16"><div className="mx-auto max-w-3xl text-center"><p className="text-xs font-bold uppercase tracking-[0.2em] text-cactus-600">Why The Desert Beneath</p><h2 className="mt-2 font-serif text-3xl font-bold text-cactus-800 sm:text-4xl">Cactus care, explained from the roots up.</h2><p className="mt-4 leading-7 text-sand-700">Learn the principles behind healthy growth instead of memorizing one-size-fits-all rules.</p></div><div className="mt-9 grid gap-5 md:grid-cols-3">{[
        ['01','Science-backed cultivation','Evidence-informed guidance that explains the biology behind light, water, soil, roots, and growth.'],
        ['02','Practical growing guidance','Clear steps you can actually use to diagnose problems, improve care, and grow healthier plants.'],
        ['03','Species-specific care','Understand how different cactus groups respond to light, water, soil, temperature, and seasonal changes.'],
      ].map(([n,t,d])=><div key={n} className="rounded-2xl border border-sand-200 bg-white p-6 shadow-sm"><span className="text-xs font-bold tracking-[0.2em] text-sand-500">{n}</span><h3 className="mt-4 font-bold text-cactus-800">{t}</h3><p className="mt-2 leading-7 text-sand-700">{d}</p></div>)}</div></section>
